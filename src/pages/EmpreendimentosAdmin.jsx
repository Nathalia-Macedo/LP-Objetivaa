...
export default function AdminEmpreendimentos() {
  ...
  const handleImageDelete = async (imageUrl, index) => {
    ...
    setLoading(true);
    try {
      const url = `${API_URL}/${editingId}/images`;
          ...
          <form onSubmit={handleSubmit} className="backdrop-blur-xl bg-white/40 border border-white/30 rounded-2xl shadow-xl p-8 space-y-6 animate-fade-in">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <input
                ...
                className="bg-white/70 border border-white/30 p-3 rounded-xl text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-400"
                required
              />
              <input
                name="caption"
                ...
                value={form.caption}
                onChange={handleChange}
                className="bg-white/70 border border-white/30 p-3 rounded-xl text-gray-700 placeholder:text-gray-400"
              />
              <input
                name="localizacao"
              ...
              <select
                ...
                value={form.status}
                onChange={handleChange}
                className="bg-white/70 border border-white/30 p-3 rounded-xl text-gray-700"
              >
              ...
              {form.observacoes.map((o, i) => (
                <div key={i} className="flex gap-2 items-center">
                  <input
                    ...
                    className="flex-1 bg-white/70 border border-white/30 p-2 rounded-xl text-gray-700 placeholder:text-gray-400"
                  />
                  {form.observacoes.length > 1 && (
                    <button type="button" onClick={() => removeObs(i)} className="text-red-600">
                      <Trash2 size={16} />
                    </button>
                  )}
                </div>
                  ...
                  {previews.map((preview, index) => (
                    <div key={index} className="relative">
                      <img
                        src={preview}
                        alt={`Preview ${index}`}
                        className="h-24 w-full object-cover rounded-xl shadow-md"
                      />
                      <button
                        type="button"
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-xl flex items-center justify-center gap-2 transition disabled:opacity-60"
            >
          ...
          <div className="relative">
            ...
            {projects.length === 0 ? (
              ...
              <div className="relative overflow-hidden rounded-xl shadow-lg">
                  ...
                  {projects.map((p) => (
                    <div key={p.id} className="min-w-full relative h-80 bg-orange-400 group">
                      ...
                      <div className="absolute top-3 right-3 flex gap-2 z-20">
                        <button onClick={() => handleEdit(p)} className="bg-white/80 p-1 rounded-full">
                          <Edit3 size={16} />
                        </button>
                ...
                <button
                  onClick={prevCard}
                  className="absolute left-0 top-1/2 -translate-y-1/2 bg-white/70 p-2 rounded-full z-30 hover:bg-white"
                >
                  <ChevronLeft />
                </button>
                <button
                  onClick={nextCard}
                  className="absolute right-0 top-1/2 -translate-y-1/2 bg-white/70 p-2 rounded-full z-30 hover:bg-white"
                >
                  <ChevronRight />
                </button>
              </div>
