      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          {translatedCards.map((card) => (
            <div
              key={card.id}
              className="relative flex-shrink-0 w-[250px] h-[350px] rounded-xl overflow-hidden group snap-start cursor-pointer"
              onClick={() => navigate(`/empreendimentos/${card.id}`)} // ✅ redirecionamento
            >
              <img
                src={card.images?.[0] || "/placeholder.jpg"}
                alt={card.name}
